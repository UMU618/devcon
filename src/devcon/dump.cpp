...
BOOL DumpDriverPackageData(_In_ LPCTSTR InfName, _In_ LPCTSTR provider) {
    DWORD Err = NO_ERROR;
    ...
    if (SetupFindFirstLine(hInf,
        ...
        (SetupGetStringField(&Context,
                             ...
                             ARRAYSIZE(InfData),
                             NULL))) {
        if (provider && 0!= _tcscmp(provider, InfData)) {
            goto failed;
        }
        FormatToStream(stdout, MSG_DPENUM_LIST_ENTRY, InfName);
        FormatToStream(stdout,MSG_DPENUM_DUMP_PROVIDER,InfData);

        //
        // UMU: Dump out the HardwareID.
        //
        TCHAR sec[MAX_INF_STRING_LENGTH];
        TCHAR hwid[MAX_INF_STRING_LENGTH];
        if (SetupFindFirstLine(hInf, INFSTR_SECT_MFG, InfData, &Context) &&
            (SetupGetStringField(&Context, 1, sec, ARRAYSIZE(sec), NULL)) &&
            (SetupGetStringField(&Context, 2, hwid, ARRAYSIZE(hwid), NULL))) {
            _tcscat_s(sec, L".");
            _tcscat_s(sec, hwid);
            if (SetupFindFirstLine(hInf, sec, NULL, &Context) &&
                (SetupGetStringField(&Context, 2, hwid, ARRAYSIZE(hwid),
                                     NULL))) {
                FormatToStream(stdout, MSG_DPENUM_DUMP_HARDWAREID, hwid);
            }
        } else {
            FormatToStream(stdout, MSG_DPENUM_DUMP_HARDWAREID_UNKNOWN);
        }
    } else {
